...
module "alb" {
  ...
  is_external                     = "false" #Internet-facing (internal = false)
  lb_type                         = "application"
  sg_enable_ssh_https             = module.security_groups.sg_ec2_sg_ssh_http_id
  ...
  lb_listner_default_action       = "forward" #Forward to target groups or redirect
...
module "hosted_zone" {
  source          = "./hosted-zone"
  domain_name     = "jenkins.techsaif.qzz.io"
