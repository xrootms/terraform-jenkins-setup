...
resource "aws_lb" "dev_proj_1_lb" {
  name               = var.lb_name
  internal           = var.is_external #Internet-facing (internal = false)
...
#attache EC2 instance (or target) to an ALB target group.
...
resource "aws_lb_listener" "dev_proj_1_lb_listner" {
  ...
  default_action {
    type             = var.lb_listner_default_action #Forward to target groups
    target_group_arn = var.lb_target_group_arn
  }
}
